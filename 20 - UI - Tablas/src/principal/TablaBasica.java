...
public class TablaBasica extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        if (tablaDatos.getColumnModel().getColumnCount() > 0) {
            tablaDatos.getColumnModel().getColumn(0).setMinWidth(15);
            ...
            tablaDatos.getColumnModel().getColumn(3).setMinWidth(20);
            tablaDatos.getColumnModel().getColumn(3).setPreferredWidth(20);
        }
    ...
    private void btnAgregarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAgregarActionPerformed
        ...
        if(vacio == -1){
            ...
            for(int j=0;listaPersonas[j]!=null; j++){
                String doc = listaPersonas[j].getDocumento();
                String name = listaPersonas[j].getNombres();
                String lastName = listaPersonas[j].getApellidos();
                String phone = listaPersonas[j].getTelefono();
                String email = listaPersonas[j].getCorreo();
                
                if(doc.equals(documento) || name.equals(nombres)|| lastName.equals(apellidos)|| phone.equals(telefono)|| email.equals(correo)){
                    duplicados=true;
                }                
            }
            
            if(duplicados==true){
                Alerta ventana2= new Alerta("El usuario ya existe");
            }else{
